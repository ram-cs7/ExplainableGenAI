...
@testset "ExplainableGenAI.jl 2026 SOTA Core Tests" begin
    ...
    @testset "Faithfulness Evaluation" begin
        # Test Logit KL Divergence
        logits_clean = [10.0f0, 0.0f0, 0.0f0]
        logits_identical = [10.0f0, 0.0f0, 0.0f0]
        logits_corrupt = [0.0f0, 10.0f0, 0.0f0]
        
        kl_zero = logit_kl_divergence(logits_clean, logits_identical)
        kl_high = logit_kl_divergence(logits_clean, logits_corrupt)
        
        @test kl_zero ≈ 0.0 atol=1e-5
        @test kl_high > 0.5
    end
