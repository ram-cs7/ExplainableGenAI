...
struct EquivariantSparseAutoencoder
    ...
    symmetry_group_matrix::AbstractMatrix # Transformation matrix T
end

Flux.@functor EquivariantSparseAutoencoder (sae,)

function EquivariantSparseAutoencoder(d_model::Int, d_dict::Int, T::AbstractMatrix; k::Int=32)
    base_sae = SparseAutoencoder(d_model, d_dict, k=k)
    return EquivariantSparseAutoencoder(base_sae, T)
end

function (esae::EquivariantSparseAutoencoder)(x::AbstractMatrix)
    # The forward pass is identical to the base SAE
    return esae.sae(x)
end
...
function (sae::JumpReLUSparseAutoencoder)(x::AbstractMatrix)
    ...
    # For Zygote: use Straight-Through Estimator (STE) — pass gradients through as if H were identity
    # Forward: hard threshold. Backward: gradients pass through via the ReLU below.
    relu_acts = Flux.relu.(pre_acts)
    # Apply hard threshold mask (STE: detach from gradient for the mask itself)
    mask = relu_acts .>= θ
    acts = relu_acts .* mask
