...
function diagnose_failure(trace::AgentTrace; agent_executor::Union{Function, Nothing}=nothing)
    ...
    if agent_executor !== nothing
        # True Counterfactual Diagnosis (AgenTracer method)
        # For each failed node, we inject a "perfect" or "corrected" output 
        # and see if the final outcome flips to :Success
        for node in trace.nodes
            if haskey(node.output_state, "error")
                # Inject a perfect output where the error is resolved
                ...
                if new_outcome == :Success
                    # Flipping this node fixed the entire trace! Absolute attribution.
                    attribution_scores[node.module_type] += 1.0
                else
                    # Partial attribution: it helped but didn't solve everything
                    attribution_scores[node.module_type] += 0.2
                end
            end
        end
    else
